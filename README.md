...
In one terminal run `npx nx serve ai-chat-server` to start the local chat server. **You will need to add your `OPENAI_API_KEY` to the `.env` file**.
In one terminal run `npx nx serve ladyeet-ai-chat` to start the Angular application.
